...
export async function getSession() {
  ...
  const sessao = await db.sessao.findUnique({ where: { token }, include: { user: { include: { turma: true } } } })
...
export async function createSession(userId: string) {
  ...
  const sessao = await db.sessao.upsert({ where: { userId }, update: { token, expiresAt }, create: { token, userId, expiresAt } })
  return sessao
}
