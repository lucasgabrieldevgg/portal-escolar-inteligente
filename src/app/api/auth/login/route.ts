...
// Login sem senha - só precisa o email (modo demonstração)
export async function POST(req: NextRequest) {
  ...
  if (!user.ativo) return NextResponse.json({ error: 'Conta desativada' }, { status: 403 })
  ...
  const res = NextResponse.json({
    user: {
      ...
      turno: user.turno, turmaId: user.turmaId, turma: user.turma,
      xp: user.xp, xpTotal: user.xpTotal, moedinhas: user.moedinhas,
      iaUsadasHoje: user.iaUsadasHoje, iaLimiteDiario: user.iaLimiteDiario,
      sequenciaDias: user.sequenciaDias, avatarConfig: user.avatarConfig,
    },
  })
...
// GET retorna contas demo para seleção (sem necessidade de senha)
export async function GET() {
  await garantirBanco()
  const users = await db.user.findMany({
    where: { ativo: true },
    select: { id: true, email: true, name: true, role: true, turno: true, turma: { select: { nome: true, ano: true } } },
